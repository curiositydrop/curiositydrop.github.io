...
async function shareCurrentPage() {
  const shareData = {
    ...
    text: 'Check this out on BANDtroductions',
  ...
  if (navigator.share) {
    try {
      ...
      if (error?.name !== 'AbortError') console.error('Could not open share options:', error);
    }
    return;
  }
...
if (window.location.pathname.endsWith('/community.html')) {
  ...
  import('./community-sponsor-slot.js?v=4').catch((error) => {
