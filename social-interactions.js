...
function addStyles(){if(document.getElementById('social-interactions-style'))return;const style=document.createElement('style');style.id='social-interactions-style';style.textContent=`
.notification-nav-link,.notification-nav-link:visited{position:relative;display:inline-flex;align-items:center;gap:5px;color:#0ccfbd!important;text-decoration:none;font-weight:800}.notification-badge{min-width:18px;height:18px;padding:0;border-radius:0;background:transparent;color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:.82rem;font-weight:900}.notification-badge[hidden]{display:inline-flex!important}#auth-account-bar #auth-logout-link,#auth-account-bar #auth-logout-link:visited{display:inline!important;padding:0!important;border:0!important;border-radius:0!important;background:transparent!important;color:#0ccfbd!important;font-weight:800!important;text-decoration:none!important}#auth-account-bar #auth-logout-link:hover,#auth-account-bar #auth-logout-link:focus{text-decoration:underline!important}.tag-helper{position:relative}.tag-suggestions{display:grid;gap:5px;margin-top:6px;padding:8px;border:1px solid #333;border-radius:12px;background:#090909}.tag-suggestions[hidden]{display:none!important}.tag-option{border:0;border-radius:9px;background:#171717;color:#fff;padding:9px 10px;text-align:left;font:inherit;cursor:pointer}.tag-option strong{color:#0ccfbd}.tag-help{display:block;color:#888;font-size:.72rem;margin-top:5px}.liker-button{border:0;background:transparent;color:#0ccfbd;text-decoration:underline;font-size:.74rem;font-weight:800;cursor:pointer;padding:5px}.liker-modal{position:fixed;inset:0;background:rgba(0,0,0,.82);z-index:12000;display:grid;place-items:center;padding:16px}.liker-modal[hidden]{display:none!important}.liker-dialog{width:min(100%,430px);max-height:75vh;overflow:auto;border:1px solid #0ccfbd;border-radius:16px;background:#111;padding:16px}.liker-dialog-head{display:flex;justify-content:space-between;gap:10px;align-items:center}.liker-dialog-head button{width:auto!important}.liker-list{display:grid;gap:8px;margin-top:12px}.liker-person{display:flex;justify-content:space-between;gap:10px;padding:10px;border:1px solid #333;border-radius:10px;color:#fff;text-decoration:none}.liker-person strong{color:#0ccfbd}
`;document.head.appendChild(style)}

function installNotificationLink(user){const bar=document.getElementById('auth-account-bar');if(!bar||document.getElementById('notification-nav-link'))return;const link=document.createElement('a');link.id='notification-nav-link';link.className='notification-nav-link';link.href='notifications.html';link.innerHTML='Notifications <span id="notification-badge" class="notification-badge">0</span>';const logout=document.getElementById('auth-logout-link');bar.insertBefore(link,logout||null);const badge=link.querySelector('#notification-badge');const q=query(collection(db,'notifications'),where('recipientId','==',user.uid),where('read','==',false));onSnapshot(q,snap=>{badge.textContent=snap.size>99?'99+':String(snap.size)},e=>console.warn('Notifications badge unavailable',e));}

async function loadProfiles(){try{const snap=await getDocs(query(collection(db,'profiles'),where('published','==',true)));profiles=snap.docs.map(d=>({id:d.id,...d.data()}));profiles.forEach(p=>profileMap.set(p.id,p))}catch(e){console.warn('Tag profiles unavailable',e)}}
