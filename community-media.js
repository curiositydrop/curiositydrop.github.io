...
function addPickerItem(rawItem){
  const item=normalizedItem(rawItem);
  if(!item.downloadUrl)return;
  const button=document.createElement('button');
  button.type='button';
  button.style.cssText='padding:0;border:1px solid #333;border-radius:10px;overflow:hidden;background:#080808;aspect-ratio:1/1;min-width:0';
  const media=item.mediaType==='video'?document.createElement('video'):document.createElement('img');
  media.src=item.downloadUrl;media.alt=item.caption||'';media.style.cssText='width:100%;height:100%;object-fit:cover;display:block';
...
async function loadPicker(){
  ...
  try{
    ...
    const profileItems=Array.isArray(profileData.mediaItems)?profileData.mediaItems:[];
    profileItems.forEach(addPickerItem);

    try{
      const q=query(collection(db,'media'),where('ownerId','==',currentUser.uid),orderBy('createdAt','desc'));
      const snap=await getDocs(q);
      snap.forEach(d=>addPickerItem({id:d.id,...d.data()}));
    }catch(libraryError){
      console.warn('Separate media library was unavailable; profile media was still loaded.',libraryError);
    }

    pickerStatus.textContent=pickerGrid.children.length?'':'You have not added any profile images or media yet.';
  }catch(error){
    console.error(error);
    pickerStatus.textContent='Your profile media could not be loaded.';
  }
}
...
publishButton.addEventListener('click',async event=>{
  ...
  if(!content){status.textContent='Write something before publishing.';return;}
  publishButton.disabled=true;status.textContent='Publishing…';
  try{
    let raw=document.getElementById('post-link').value.trim();
    const linkUrl=raw?(/^https?:\/\//i.test(raw)?raw:`https://${raw}`):'';
    await addDoc(collection(db,'posts'),{authorId:currentUser.uid,authorName:profile.displayName||currentUser.displayName||'BANDtroductions Member',accountType:profile.accountType||'member',category:document.getElementById('post-category').value,content,linkUrl,imageUrl:selectedMedia.downloadUrl,mediaId:selectedMedia.id,mediaType:selectedMedia.mediaType,published:true,createdAt:serverTimestamp(),updatedAt:serverTimestamp()});
    document.getElementById('post-content').value='';document.getElementById('post-link').value='';selectedMedia=null;preview.hidden=true;preview.replaceChildren();status.textContent='Posted.';fields.hidden=true;document.getElementById('composer-toggle').setAttribute('aria-expanded','false');document.getElementById('composer-toggle').textContent='Create Post';
  }catch(error){console.error(error);status.textContent=error.code==='permission-denied'?'Post or media permissions are not enabled yet.':'Your post could not be published.';}finally{publishButton.disabled=false;}
},true);
...
onAuthStateChanged(auth,async user=>{
  ...
  if(mediaId){
    try{const snap=await getDoc(doc(db,'media',mediaId));if(snap.exists()&&snap.data().ownerId===user.uid){showSelected({id:snap.id,...snap.data()});document.getElementById('composer-fields').hidden=false;document.getElementById('composer-toggle').setAttribute('aria-expanded','true');document.getElementById('composer-toggle').textContent='Close';}}catch(error){console.error(error);}
  }
});